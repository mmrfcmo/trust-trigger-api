...
async def create_project_from_lead(lead_id: str, db: AsyncSession = Depends(get_db)):
    ...
    tasks_data = [
        {"type": "website_scan", "depends_on": [], "order": 1},
        {"type": "google_business_scan", "depends_on": [], "order": 2},
        {"type": "trust_score", "depends_on": ["website_scan", "google_business_scan"], "order": 3},
        {"type": "evidence_capture", "depends_on": ["website_scan"], "order": 4},
        {"type": "homepage_copy", "depends_on": ["trust_score"], "order": 5, "status": "awaiting_review"},
        {"type": "email_sequence", "depends_on": ["trust_score"], "order": 6, "status": "awaiting_review"},
        {"type": "google_business_posts", "depends_on": ["trust_score"], "order": 7, "status": "awaiting_review"},
        {"type": "social_media_posts", "depends_on": ["trust_score"], "order": 8, "status": "awaiting_review"},
        {"type": "trust_snapshot_report", "depends_on": ["trust_score"], "order": 9},
    ]
    task_ids = {}
    for td in tasks_data:
        ...
        db.add(t)
        await db.flush()
        task_ids[td["type"]] = t.id
    await db.commit()
    return {"success": True, "project_id": project.id, "lead_id": lead_id, "tasks_created": len(tasks_data)}
