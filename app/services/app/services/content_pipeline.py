...
async def generate_email_sequence(task, project, scan_data, score_data):
    emails = [
        {"subject": f"Thanks for reaching out, {project.business_name}!", "body": f"Hi there,\n\nThanks for your interest. We'll be in touch shortly."},
        {"subject": f"Meet the team", "body": f"Hi there,\n\nWe wanted to introduce ourselves."},
        {"subject": "What our customers are saying", "body": f"Hi there,\n\nHere's what our customers say about us."},
        {"subject": "Frequently asked questions", "body": f"Hi there,\n\nHere are answers to common questions."},
        {"subject": "Ready to get started?", "body": f"Hi there,\n\nReady to take the next step? Contact us today."},
    ]
...
async def generate_gbp_posts(task, project, scan_data, score_data):
    posts = [f"Welcome to {project.business_name}! We're proud to serve our community.",
             ...
             f"Ready to experience the {project.business_name} difference? Contact us!",
...
async def run_content_generator(task, project):
    ...
    try:
        output = await generator(task, project, scan_data, score_data)
